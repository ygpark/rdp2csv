# .evtx 파일 경로
$filePath = "Microsoft-Windows-RemoteDesktopServices-RdpCoreTS%4Operational.evtx"  # 실제 파일 경로로 변경 필요
...
try {
    ...
    $events = Get-WinEvent -FilterHashtable @{Path = $filePath; Id = 131, 140 } -ErrorAction Stop
}
catch {
